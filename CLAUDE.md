...
8. Phase 0 is prep on shipped code and must precede Phase 9. Phases 7 and 8 are unsequenced and can be built at any point. Phase 16 (OCR-assisted entry) is optional — build it only if explicitly requested. The apps track (`phases/apps/`) is numbered in build order: 31 → 32 → 33 → 34; 35–37 (the companion phone app and its transports, under `phases/apps/parked/`) are parked — phones use a browser — and if un-parked are built in number order; see `phases/apps/README.md`.
...
- `phases/apps/` — the phases that make the server something people install: a desktop app, and a QR code so every phone in the house reaches it in a browser. `phases/apps/README.md` has the one-paragraph design and the reading order, `phases/apps/BUILD-BRIEF.md` the session prompts; the constraint they all share is that nothing outside the home network is ever in the default path.
  Numbers 19, 20, 28, 29 and 30 were this track's original numbers and are retired; git history before 2026-09-15 uses them.
  - `phases/apps/phase-31-pwa-install-flow.md` (shipped — manifest and icons so the web app pins to a home screen, the network-first shell cache in `web/public/sw.js` — read its header before touching it — and the reconnect banner worded by `web/src/reachability.ts`; built over plain HTTP. Every connectivity failure is reported from `api.ts`'s `onServerUnreachable`, never from a page; recovery remounts the current view through `reconnectEpoch` in `App.tsx`)
  - `phases/apps/phase-32-desktop-app.md` (shipped — Electron shell around the unmodified `server/dist` on a bundled official Node, never Electron's; close hides, Quit stops, on every platform; LAN sharing off by default; the port remembered in the shell's own config. Two server edits: the worker picks `.ts` vs `.js` by its own extension, and `MTG_SHUTDOWN_ON_STDIN_CLOSE=1` lets the shell stop the server by closing its stdin because Windows has no SIGTERM. macOS arm64 verified; the Windows x64 installer builds from the same workspace and its runtime walkthrough is in the doc)
  - `phases/apps/phase-33-pairing-and-lan-discovery.md` (shipped — `instance_id` minted in `db/index.ts` (get-or-create, so a restore of an older backup re-mints it), `GET /api/v1/instance`, `_mtglibrary._tcp` over `bonjour-service` gated by `MTG_ADVERTISE=1` *and* a non-loopback bind (`server/src/discovery/`), and the QR on the Data page (`PairPhonePanel.tsx`, `pairing.ts`). One rule added at build time: the QR's host is the address the page itself was reached on when that is a home-network IPv4, ahead of the server's list — behind Docker's bridge the server only knows `172.17.0.2`. Home network only, deliberately — no tunnels, relays or embedded VPNs; "bring your own Tailscale" stays a documented advanced option. The "Phone: discovery order" section is the parked companion app's contract, not built)
  - `phases/apps/phase-34-signing-and-updates.md` (after 32 — Developer ID signing and notarisation, Windows signing when a cert exists, `electron-updater` against GitHub Releases, the `desktop.yml` release workflow on the same `v*` tag as Docker)
