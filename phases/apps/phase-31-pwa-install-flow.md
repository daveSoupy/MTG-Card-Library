...
> - **The worker also prunes** `/assets/*` entries the freshly fetched `index.html` no longer references (the doc's optional step), skips `.map` files, and does its cache write inside `event.waitUntil` so the page is never made to wait for it.
...
> - **Verification 2, 4, 5 and 7 were run in real Chrome via Playwright** against a production build on a copy of the database — the desktop app's embedded browser pane cannot register service workers. Item 3 and the phone halves of 4 and 5 are manual on a phone.
