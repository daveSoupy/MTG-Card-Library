...
Phase 31 (PWA install) is not a prerequisite and is not replaced — a phone without this app still gets the web app from the same QR code.
...
- **Home mode** — Phase 33's discovery found the paired server. The app is the web app, unchanged, against the live server. Deck building, collection, everything. This is the same client as the browser; it must never grow a branch of its own.
...
Each trigger runs Phase 33 discovery first; "not home" is a silent no-op, not an error. A `4xx` other than a replayed-key hit marks the item failed with the server's message and moves on; failures are listed in the app, never silently dropped.
...
- **Background sync.** iOS `BGTaskScheduler` + background `URLSession`; Android `WorkManager`. Safari has no Background Sync API and never will, which is the single strongest reason this is an app and not the PWA. The task runs Phase 33 discovery, and if home, pushes the queue and pulls a snapshot.
...
The desktop app (Phase 32) and this app update independently, so either can be ahead. Two contracts, additive-only from the day they ship:
...
The app reads `serverVersion` from Phase 33's instance endpoint and shows *Update the desktop app* rather than failing oddly when the server is older than the oldest version it supports.
...
- Live access from outside the home network. Phase 33 explains why; `deploy/README.md` keeps "bring your own Tailscale" for anyone who wants it anyway.
